...
- **ts-jest runs transpile-only** (`isolatedModules: true` in tsconfig.json). Types are ERASED before Jest executes, so a test that only annotates a type proves nothing — it passes identically whether or not the type is correct. Never write a test whose only assertion is a type annotation (`const t: SomeUnion = 'X'; expect(t).toBe('X')`). Type correctness is verified by `npx tsc --noEmit`, never by Jest. Every Jest test must assert runtime behaviour.
