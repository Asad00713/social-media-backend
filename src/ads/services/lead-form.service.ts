...
export class LeadFormService {
  ...
  async create(workspaceId: string, channelId: number, dto: CreateLeadFormDto) {
    ...
    if (channel.platform !== 'facebook')
      throw new BadRequestException(
        'Lead forms require a Facebook Page channel',
      );
    if (!channel.accessToken)
      throw new BadRequestException('No page access token for channel');
    ...
    const remote = await this.meta.createLeadForm(
      ...
      pageToken,
      {
        ...
        privacy_policy: {
          url: dto.privacyPolicy.url,
          link_text: dto.privacyPolicy.linkText,
        },
        questions: dto.questions.map((q) => {
          if (q.type === 'CUSTOM') {
            if (!q.key || !q.label)
              throw new BadRequestException(
                'Custom questions require key + label',
              );
            return {
  ...
  async list(workspaceId: string) {
    return db
      .select()
      .from(leadForms)
      .where(eq(leadForms.workspaceId, workspaceId));
  }
}
