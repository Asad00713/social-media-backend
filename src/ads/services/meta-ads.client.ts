...
import type {
  ...
  MetaCampaignCreate,
  MetaAdSetCreate,
  MetaCreativeBoostInput,
  MetaCreativeBoostOverridesInput,
  MetaCreativeLeadInput,
  MetaLeadFormInput,
  MetaInsightsRow,
  MetaTargeting,
...
export class MetaAdsClient {
  ...
  async listAdAccounts(userAccessToken: string): Promise<MetaAdAccountDto[]> {
    const fields =
      'id,account_id,name,currency,timezone_name,account_status,disable_reason,business,capabilities,funding_source_details';
  ...
  async uploadAdImage(
    ...
    const res = await fetch(`${GRAPH}/act_${stripAct(adAccountId)}/adimages`, {
      method: 'POST',
      body,
    });
    ...
    if (!res.ok || !data.images) {
      throw new BadRequestException(
        `Meta adimage upload failed: ${data.error?.message ?? JSON.stringify(data)}`,
      );
    }
  ...
  async createCreativeBoost(
    adAccountId: string,
    token: string,
    input: MetaCreativeBoostInput,
  ): Promise<{ id: string }> {
  ...
  async createCreativeBoostWithOverrides(
    ...
    return this.postJson(
      `/act_${stripAct(adAccountId)}/adcreatives`,
      token,
      input,
    );
  }

  async createCreativeLead(
    adAccountId: string,
    token: string,
    input: MetaCreativeLeadInput,
  ): Promise<{ id: string }> {
    return this.post(`/act_${stripAct(adAccountId)}/adcreatives`, token, input);
  }
  ...
  async getLead(
    ...
    return this.get(
      `/${leadgenId}?fields=id,created_time,field_data,form_id,ad_id,adset_id,campaign_id,is_organic&access_token=${encodeURIComponent(pageAccessToken)}`,
    );
  }
  ...
  async getInsights(
    ...
    const tr = timeRange
      ...
      : '';
    const fields =
      'impressions,reach,clicks,spend,cpc,cpm,ctr,actions,date_start,date_stop';
  ...
  async updateAdSetBudget(
    ...
    return this.post(`/${metaAdsetId}`, token, {
  ...
  > {
    ...
    const res = await this.get<{ data: Array<{ id: string; name: string }> }>(
      url,
    );
    return res.data ?? [];
  }
  ...
  async searchLanguages(
    ...
    const res = await this.get<{ data: Array<{ key: number; name: string }> }>(
      url,
    );
    return res.data ?? [];
  }
  ...
  private async get<T>(path: string): Promise<T> {
    ...
    if (!res.ok) {
      ...
      this.logger.error(
        `Meta GET ${sanitized} failed: ${JSON.stringify(data)}`,
      );
      throw new BadRequestException(
        `Meta API error: ${data?.error?.message ?? 'unknown'} (code: ${data?.error?.code})`,
      );
    }
    return data as T;
  }
  ...
  private async postJson<T>(
    ...
    const res = await fetch(
      `${GRAPH}${path}?access_token=${encodeURIComponent(token)}`,
      {
        method: 'POST',
    ...
    if (!res.ok) {
      this.logger.error(
        `Meta POST(json) ${path} failed: ${JSON.stringify(data)} body: ${JSON.stringify(body)}`,
      );
      const err = data?.error ?? {};
      const detail =
        err.error_user_msg || err.error_user_title || err.message || 'unknown';
      throw new BadRequestException(
  ...
  private async post<T>(path: string, token: string, body: object): Promise<T> {
    ...
    if (!res.ok) {
      this.logger.error(
        `Meta POST ${path} failed: ${JSON.stringify(data)} body: ${JSON.stringify(body)}`,
      );
      const err = data?.error ?? {};
      let detail =
        err.error_user_msg || err.error_user_title || err.message || 'unknown';
