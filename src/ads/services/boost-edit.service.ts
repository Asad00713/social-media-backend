...
export class BoostEditService {
  ...
  async apply(workspaceId: string, campaignId: string, dto: BoostEditDto) {
    const editableKeys: Array<keyof BoostEditDto> = [
      'dailyBudgetMinor',
      'startTime',
      'endTime',
      'audience',
    ...
    if (campaign.kind !== 'boost')
      throw new BadRequestException(
        'Only boost campaigns can be edited via this endpoint',
      );
    ...
    const [adset] = await db
      ...
      .limit(1);
    if (!adset)
      throw new NotFoundException('Ad set not found for this campaign');
    ...
    if (channel.platform !== 'facebook')
      throw new BadRequestException(
        'Only FB Page boosts can be edited via this endpoint',
      );
    ...
    if (dto.dailyBudgetMinor !== undefined) {
      await this.meta.updateAdSetBudget(
        adset.metaAdsetId,
        userToken,
        dto.dailyBudgetMinor,
      );
      await db
        .update(adSets)
        .set({ dailyBudgetMinor: dto.dailyBudgetMinor, updatedAt: new Date() })
      ...
      await db
        ...
        .set({
          ...
          ...(dto.endTime !== undefined
            ? { endTime: dto.endTime === null ? null : new Date(dto.endTime) }
            : {}),
          updatedAt: new Date(),
        })
        .where(eq(adSets.id, adset.id));
    ...
    if (dto.audience) {
      ...
      await this.meta.updateAdSetTargeting(
        adset.metaAdsetId,
        userToken,
        targeting,
      );
    ...
    const editedFields = Object.keys(dto).filter(
      (k) => k !== 'acknowledgeLearningPhaseReset',
    );
