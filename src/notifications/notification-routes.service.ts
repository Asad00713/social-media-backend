...
import type { CreateNotificationRouteDto, UpdateNotificationRouteDto } from './dto/notification-route.dto'

@Injectable()
export class NotificationRoutesService {
  list(workspaceId: string) {
    return db.select().from(notificationRoutes).where(eq(notificationRoutes.workspaceId, workspaceId))
  }
  ...
  async create(workspaceId: string, dto: CreateNotificationRouteDto) {
    ...
    const [channel] = await db
      ...
      .where(and(eq(socialMediaChannels.id, dto.channelId), eq(socialMediaChannels.workspaceId, workspaceId)))
      .limit(1)
    if (!channel) throw new ForbiddenException('Channel does not belong to this workspace')
  ...
  async update(workspaceId: string, id: string, dto: UpdateNotificationRouteDto) {
    const [existing] = await db
      .select()
      .from(notificationRoutes)
      .where(and(eq(notificationRoutes.id, id), eq(notificationRoutes.workspaceId, workspaceId)))
      .limit(1)
    ...
    const [row] = await db
      ...
      .set({
        ...
        ...(dto.targetPlatformChannelId !== undefined ? { targetPlatformChannelId: dto.targetPlatformChannelId } : {}),
        ...(dto.targetDisplayName !== undefined ? { targetDisplayName: dto.targetDisplayName } : {}),
