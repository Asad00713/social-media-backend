...
export const notificationRoutes = pgTable(
  'notification_routes',
  {
    ...
    channelId: integer('channel_id').notNull().references(() => socialMediaChannels.id, { onDelete: 'cascade' }),
    targetPlatformChannelId: varchar('target_platform_channel_id', { length: 128 }).notNull(),
  ...
  (t) => ({
    ...
    byEvent: index('notification_routes_event_idx').on(t.workspaceId, t.eventType),
  }),
)
