...
export const PROFILE_SYNC_STATUSES = [
  'success',
  'failed',
  'rate_limited',
] as const;
...
export const channelSyncState = pgTable('channel_sync_state', {
  ...
  lastProfileSyncStatus: varchar('last_profile_sync_status', {
    length: 20,
  }).$type<ProfileSyncStatus>(),
  lastProfileSyncError: text('last_profile_sync_error'),
  nextProfileSyncAt: timestamp('next_profile_sync_at', {
    withTimezone: true,
  }).notNull(),
  ...
  pubsubhubbubLeaseExpiresAt: timestamp('pubsubhubbub_lease_expires_at', {
    withTimezone: true,
  }),
});
