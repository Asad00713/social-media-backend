...
export class YouTubeAnalyticsAdapter implements PlatformAnalyticsAdapter {
  ...
  estimateQuotaCost(op: AdapterOperation): number {
    switch (op) {
      case 'fetchProfileSnapshot':
        return 2;
      case 'fetchPostMetrics':
        return 1;
  ...
  async fetchProfileSnapshot(
    ...
    try {
      const response = await this.dataClient.getChannelById(
      ...
      return {
        status: 'success',
        data: {
          followersCount: stats.hiddenSubscriberCount
            ? null
  ...
  async fetchPostMetrics(post: PostEntity): Promise<PostMetricsResult> {
    try {
      ...
      const videoResp = await this.dataClient.getVideosByIds(
        [videoId],
        accessToken,
      );
      ...
      let analyticsRow: Awaited<
        ReturnType<typeof this.analyticsClient.getVideoMetrics>
      > = null;
      ...
      try {
        ...
        this.logger.warn(
          `Analytics API unavailable for video ${videoId}: ${(analyticsErr as Error).message}`,
        );
      }
      ...
      const data = {
        ...
        commentsCount: Number(
          dataStats.commentCount ?? analyticsRow?.comments ?? 0,
        ),
        sharesCount: analyticsRow?.shares ?? null,
        impressionsCount: Number(
          dataStats.viewCount ?? analyticsRow?.views ?? 0,
        ),
        reachCount: null,
        platformMetrics: {
          ...
          averageViewDurationSeconds:
            analyticsRow?.averageViewDurationSeconds ?? null,
  ...
  async fetchRecentPosts(
    ...
    try {
      const searchResp = await this.dataClient.listChannelVideos(
        channel.platformAccountId,
        channel.accessToken,
        {
      ...
      if (videoIds.length === 0)
        return { status: 'success', data: { posts: [] }, quotaCostUsed: 100 };

      const videosResp = await this.dataClient.getVideosByIds(
        videoIds,
        channel.accessToken,
      );
  ...
  private toFailedResult(
    ...
    const code: AdapterError['code'] =
      ytErr && ytErr.code ? ytErr.code : 'transient';
