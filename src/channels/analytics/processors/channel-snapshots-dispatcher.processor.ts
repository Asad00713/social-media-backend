...
import {
  ...
  type ChannelProfileSnapshotJob,
} from '../handlers/channel-profile-snapshot.handler';
import {
  PostMetricSnapshotHandler,
  type PostMetricSnapshotJob,
} from '../handlers/post-metric-snapshot.handler';
import {
  ChannelDailyRollupHandler,
  type ChannelDailyRollupJob,
} from '../handlers/channel-daily-rollup.handler';
import {
  ChannelInitialBackfillHandler,
  type ChannelInitialBackfillJob,
} from '../handlers/channel-initial-backfill.handler';
import {
  ChannelRecentPostsSyncHandler,
  type ChannelRecentPostsSyncJob,
} from '../handlers/channel-recent-posts-sync.handler';
...
export class ChannelSnapshotsDispatcherProcessor extends WorkerHost {
  private readonly logger = new Logger(
    ChannelSnapshotsDispatcherProcessor.name,
  );
  ...
  async process(job: AnyJob): Promise<{ ok: boolean }> {
    switch (job.name) {
      ...
      default:
        this.logger.warn(
          `Unknown job name on channel-snapshots queue: ${job.name}`,
        );
        return { ok: true };
    }
  }
