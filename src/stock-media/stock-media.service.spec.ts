  ...
  results: [
    {
      ...
      links: {
        self: 's',
        html: 'https://unsplash.com/photos/a1',
        download: 'd',
        downloadLocation: 'https://api.unsplash.com/photos/a1/download',
      },
      user: {
        id: 'u',
        username: 'x',
...
describe('StockMediaService.search', () => {
  ...
  it('derives hasMore=false on the last unsplash page', async () => {
    const { service } = makeService({
      unsplash: {
        searchPhotos: jest
          .fn()
          .mockResolvedValue({ ...unsplashResult, totalPages: 1 }),
      },
    });
    const res = await service.search({
      provider: 'unsplash',
    ...
    const { service, pexels } = makeService({
      pexels: {
        searchPhotos: jest.fn().mockResolvedValue({
          ...
          page: 1,
          perPage: 24,
          nextPage: 'https://api.pexels.com/next',
          prevPage: null,
        }),
      },
    });
  ...
  it('maps pexels videos', async () => {
    ...
    expect(pexels.searchVideos).toHaveBeenCalledWith({
      query: 'cats',
      page: 1,
      perPage: 24,
    });
...
describe('StockMediaService.track', () => {
  it('delegates unsplash download-location hosts to the unsplash service', async () => {
    ...
    expect(unsplash.trackDownload).toHaveBeenCalledWith(
      'https://api.unsplash.com/photos/a1/download?ixid=1',
    );
  });
  ...
  it('does not throw when the unsplash trackDownload ping rejects', async () => {
    const { service, unsplash } = makeService({
      unsplash: {
        trackDownload: jest.fn().mockRejectedValue(new Error('boom')),
      },
    });
    await expect(
      service.track('https://api.unsplash.com/x/download'),
